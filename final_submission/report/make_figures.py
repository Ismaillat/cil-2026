...
fig, ax = plt.subplots(figsize=(3.3, 2.0))
xs = np.arange(len(ORDER))
en_scores = [per_lang[m][0] for m in ORDER]
de_scores = [per_lang[m][1] for m in ORDER]
ax.bar(xs - 0.2, en_scores, 0.4, label='English', color='#3b6fb5')
ax.bar(xs + 0.2, de_scores, 0.4, label='German', color='#c4302b')
ax.set_xticks(xs)
ax.set_xticklabels([MODEL_LABELS[m] for m in ORDER], rotation=45, ha='right')
ax.set_ylabel('validation score')
ax.set_ylim(0.70, 0.93)
ax.legend(loc='lower left', frameon=False)
ax.grid(axis='y', alpha=0.25)
fig.tight_layout()
fig.savefig(os.path.join(FIGDIR, 'per_lang_scores.pdf'), bbox_inches='tight')
plt.close(fig)
print('wrote per_lang_scores.pdf')
...
prog = [
    ...
    ('DeBERTa-EN + gBERT-DE (hybrid)', 0.911),
    ('Weighted-MAE per-language (10 models)', 0.912),
]
