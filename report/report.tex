...
\textbf{Error analysis.} % TODO: fill from final-model confusion matrix.
The confusion matrix concentrates mass on the diagonal and its immediate
neighbors, confirming that residual errors are predominantly off-by-one---the
expected behavior for a calibrated ordinal model and the cheap kind of error
under MAE. The costly extreme confusions (one star vs.\ five stars) are rare
and, on inspection, dominated by sarcastic or internally contradictory
reviews (e.g.\ praise followed by a complaint about shipping), which are
genuinely ambiguous even to a human reader.
...
commodity hardware. % TODO: one closing sentence with the final score once
% the ensemble run completes.
