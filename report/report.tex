...
An ensemble of two diverse base encoders reaches a validation score of
$0.908$ ($1-\mathrm{MAE}/4$), exceeding the strong baseline while remaining
trainable on a single commodity GPU.
\end{abstract}
...
mDeBERTa-v3-base (fine-tuned)     & 0.907 & TODO \\
\textbf{Ensemble (ours)}          & \textbf{0.908} & \textbf{TODO} \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
\textbf{Frozen embeddings underperform lexical features.} The linear probe
(B2) scores below the sparse TF--IDF baseline (B1), which at first seems
counter-intuitive: contextual embeddings ``should'' beat a bag of words. The
explanation is that the sentence encoder was pre-trained for
\emph{paraphrase similarity}, not sentiment---it places ``great product''
and ``decent product'' nearby in embedding space even though they predict
different ratings. TF--IDF, by contrast, exposes the most discriminative
tokens (e.g.\ \textit{terrible}, \textit{perfekt}, and even literal phrases
like \textit{one star}) directly to the classifier. The gap is therefore not
evidence against contextual representations but against using them
\emph{frozen}: it motivates fine-tuning, which is exactly what closes the gap
and then surpasses both baselines (\cref{tab:main}).

\textbf{Task adaptation, not scale, is the lever on this hardware.} We
attempted to push performance with a larger single encoder
(XLM-RoBERTa-large) but found it impractical on our GPUs: half-precision
training diverged (the loss rose above its random-initialization value and
gradient norms exploded), the wider-range \texttt{bfloat16} format is
unsupported on the available Turing cards, and full-precision training does
not fit in $11$\,GB once optimizer state is included. Rather than fight this,
we obtained a comparable gain by ensembling two \emph{architecturally
diverse} base encoders---a strictly cheaper and more stable route to the
same end (\cref{sec:ensemble}).

\textbf{Error analysis.} On the held-out set the ensemble confusion matrix
concentrates mass on the diagonal and its immediate neighbors: across all
five classes, extreme confusions (predicting one star for a five-star review
or vice versa) number in the single digits per cell, while essentially all
errors are off-by-one. This is precisely the behavior MAE rewards---the
remaining mistakes are the cheap kind. Inspecting the rare large errors, they
are dominated by sarcastic or internally contradictory reviews (e.g.\ praise
followed by a complaint about shipping), which are genuinely ambiguous even to
a human reader and place a natural ceiling on this metric.

\section{Summary}
\label{sec:summary}
We studied ordinal sentiment classification of bilingual product reviews
under an MAE metric. A controlled comparison showed that fine-tuning a
multilingual encoder---not richer frozen features or a larger model---is the
decisive factor, and that two metric-aware techniques add further gains at
negligible cost: median decoding matched to the absolute-error metric, and a
diversity ensemble that sidesteps the instability of large-model training on
commodity hardware. Together these reach a validation score of $0.908$,
above the strong baseline, using only two fine-tuned base encoders on a
single $11$\,GB GPU.
